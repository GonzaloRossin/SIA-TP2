...
def multilayer_perceptron(X, Y, input_handler: InputHandler):
    ...
    for i in range(input_handler.num_epochs):
        ...
        for minibatch in minibatches:
            ...
            total_error += compute_error(O, minibatch_Y, input_handler.output_activation)
        ...
        errors.append(total_error/num_examples)
        if (i % 100 == 0):
            print(f"Epoch #{i}")
        '''
        if (i % 1000 == 0):
            input_handler.learning_rate = input_handler.learning_rate / (1 + 0.001 * i)
        '''
...
def multilayer_perceptron(X, Y, input_handler):
    ...
    for i in range(0, input_handler.num_epochs):
        ...
        parameters = update_params_gd(parameters, gradients, input_handler.learning_rate, input_handler.apply_bias)
